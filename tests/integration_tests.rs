...
fn test_library_integration_basic() {
    ...
    assert_eq!(relay.config().bitcoin_rpc_auth.username, "user");
    assert_eq!(relay.config().bitcoin_rpc_auth.password, "password");
...
fn test_library_integration_multiple_relays() {
    ...
    let relay1 = BitcoinNostrRelay::new(RelayConfig::for_network(Network::Regtest, 1)).unwrap();
    let relay2 = BitcoinNostrRelay::new(RelayConfig::for_network(Network::Regtest, 2)).unwrap();
    
    // Each should have unique configuration
    assert_eq!(relay1.config().relay_id, "1");
    assert_eq!(relay1.config().bitcoin_rpc_url, "http://127.0.0.1:18332");
    assert_eq!(relay1.config().strfry_url, "ws://127.0.0.1:7777");
    
    assert_eq!(relay2.config().relay_id, "2");
    assert_eq!(relay2.config().bitcoin_rpc_url, "http://127.0.0.1:18444");
    assert_eq!(relay2.config().strfry_url, "ws://127.0.0.1:7778");
}

#[test]
fn test_library_integration_testnet4() {
    // Test testnet4 configuration integration
    let relay = BitcoinNostrRelay::new(RelayConfig::for_network(Network::Testnet4, 1)).unwrap();
    
    assert_eq!(relay.config().relay_id, "1");
    assert_eq!(relay.config().bitcoin_rpc_url, "http://127.0.0.1:48330");
    assert_eq!(relay.config().strfry_url, "ws://127.0.0.1:7777");
    assert_eq!(relay.config().bitcoin_rpc_url, "http://127.0.0.1:48330");
}
...
async fn test_library_integration_validation_flow() {
    ...
    let config_disabled = RelayConfig::for_network(Network::Regtest, 1)
...
fn test_library_integration_builder_pattern() {
    ...
    let config = RelayConfig::new(
        "http://127.0.0.1:20000".to_string(),
    ...
    assert_eq!(relay.config().strfry_url, "ws://127.0.0.1:8888");
    assert_eq!(relay.config().bitcoin_rpc_auth.username, "custom_user");
    assert_eq!(relay.config().bitcoin_rpc_auth.password, "custom_password");
    assert_eq!(relay.config().mempool_poll_interval.as_secs(), 10);
    assert_eq!(relay.config().bitcoin_rpc_url, "http://127.0.0.1:20000");
}
...
async fn test_library_integration_error_handling() {
    ...
    let config_no_precheck = RelayConfig::for_network(Network::Regtest, 1)
        .with_validation(validation_config);
...
#[test]
fn test_new_convenience_method_pattern() {
    // Test the new convenience method that follows mature Rust library patterns
    let relay = BitcoinNostrRelay::new(
        RelayConfig::for_network(Network::Regtest, 1)
            .with_auth("test_user".to_string(), "test_pass".to_string())
            .with_mempool_poll_interval_secs(3)
    ).unwrap();
    
    // Verify configuration is correctly applied
    assert_eq!(relay.config().relay_id, "1");
    assert_eq!(relay.config().bitcoin_rpc_url, "http://127.0.0.1:18332");
    assert_eq!(relay.config().strfry_url, "ws://127.0.0.1:7777");
    assert_eq!(relay.config().bitcoin_rpc_auth.username, "test_user");
    assert_eq!(relay.config().bitcoin_rpc_auth.password, "test_pass");
    assert_eq!(relay.config().mempool_poll_interval.as_secs(), 3);
    
    // Compare with the functional style - both should produce same result
    let relay_functional = BitcoinNostrRelay::new(
        network_config(Network::Regtest, 1)
            .with_auth("test_user".to_string(), "test_pass".to_string())
            .with_mempool_poll_interval_secs(3)
    ).unwrap();
    
    // Both approaches should yield identical configurations
    assert_eq!(relay.config().relay_id, relay_functional.config().relay_id);
    assert_eq!(relay.config().bitcoin_rpc_url, relay_functional.config().bitcoin_rpc_url);
    assert_eq!(relay.config().bitcoin_rpc_auth.username, relay_functional.config().bitcoin_rpc_auth.username);
}